...
(use-package! roc-ts-mode
  ...
  ;; Formatting
  (set-formatter! 'roc-ts-format '("roc" "fmt" "--stdin") :modes '(roc-ts-mode))
...
Tree sitter is used to make some basic indentation rules that aim to be consistent with "roc fmt", and there are tests to check that consistency. It's still a work in progress, though.
...
|--------------------+-----------------------------+---------------+--------------------------------------------------------|
| ~C-c C-f~          | ~roc-ts-format~             | ~roc fmt~     | Format the current buffer                              |
